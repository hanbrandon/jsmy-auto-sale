...
export const InstagramGallery = ({ initialPosts = [] }: InstagramGalleryProps) => {
  ...
  const dynamicGallery = initialPosts.map((post, index) => ({
    ...
    size: index === 0 || index === 4 ? "large" : "small" as "large" | "small"
  }));
  ...
  const nextImage = () => {
    ...
    const currentIndex = gallery.findIndex(item => item.id === selectedId);
            ...
            {gallery.map((item, i) => (
              <motion.div
                ...
                className={cn(
                  ...
                  "w-[85vw] md:w-auto h-[450px] md:h-auto", // Mobile card height/width
                  item.size === "large" ? "md:col-span-2 md:row-span-2" : "md:col-span-1 md:row-span-1"
                )}
              >
                <div className="absolute inset-0 z-10 bg-white/0 group-hover:bg-white/[0.02] transition-colors duration-500" />
                ...
                {/* Overlay Content */}
                <div className="absolute inset-0 z-20 bg-gradient-to-t from-black/90 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col justify-end p-8 md:p-10">
                  <div className="flex justify-between items-end gap-4">
                    <div className="flex-1">
                      <motion.p 
                        className="text-white font-bold text-xl md:text-2xl leading-tight transform translate-y-6 group-hover:translate-y-0 transition-transform duration-500"
                      >
                        {item.caption}
                      </motion.p>
                      <div className="flex items-center gap-2 mt-3 transform translate-y-6 group-hover:translate-y-0 transition-transform duration-500 delay-75">
                        <InstagramIcon size={12} className="text-white/60" />
            ...
            <div className="relative z-10 w-full h-full flex flex-col items-center justify-center">
              ...
              <div className="relative w-full max-w-6xl aspect-video md:aspect-auto md:h-[80vh] flex items-center justify-center group">
                ...
                <motion.div
                  ...
                  className="relative w-full h-full overflow-hidden rounded-[2rem] shadow-2xl bg-white/5"
                >
                  <Image 
              ...
              >
                <h3 className="text-2xl md:text-3xl font-bold mb-2">{selectedImage.caption}</h3>
                <p className="text-white/40 text-sm font-bold uppercase tracking-widest">#JSMYAuto #PremiumDelivery</p>
              </motion.div>
            </div>
          </motion.div>
        )}
